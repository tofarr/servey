...
from servey.servey_strawberry.handler_filter.authorization_handler_filter import AuthorizationHandlerFilter
...
class TestAuthorizationHandlerFilter(TestCase):
    ...
    def filtered_action(self):
        @action(access_control=ScopeAccessControl('root', 'root'))
        ...
        expected_params = [
            ...
            inspect.Parameter("strawberry_info", inspect.Parameter.KEYWORD_ONLY, annotation=Info)
        ]
    ...
    def test_filter(self):
        ...
        request = Request(dict(
            method='GET',
            type='http',
            headers=[['authorization'.encode('latin-1'), f'Bearer {token}'.encode('latin-1')]]
        ))
