...
class HttpAction(ActionABC):
    ...
    def Execute_async(self, authorization: Authorization, **kwargs):
    ...
    def get_response(self, authorization: Authorization, kwargs) -> Any:
        ...
        if self.method == "GET":
            return requests.get(
                ...
                auth=self.auth_to_header(authorization),
            )
        else:
            return requests.post(
                self.url, auth=self.auth_to_header(authorization), json=params
            )
