...
class LocalAuthenticatorFactory(AuthenticatorFactoryABC):
    ...
    def create_authenticator(self) -> AuthenticatorABC:
        from servey.integration.fastapi_integration.authenticator.local_authenticator import LocalAuthenticator
        ...
        if password is None:
            password = base64.b64encode(os.urandom(12)).decode('UTF-8').replace('+', '').replace('/', '')
