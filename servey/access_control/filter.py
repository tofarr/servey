...
from servey.action import Action, action, get_marshaller_for_params, get_schema_for_params
...
def action_without_kwarg(action_: Action, kwarg_name: str):
    ...
    sig = sig.replace(parameters=tuple(p for p in sig.parameters.values() if p.name != kwarg_name))
