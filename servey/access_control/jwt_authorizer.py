...
class JwtAuthorizer(AuthorizerABC):
    ...
    def encode(self, authorization: Authorization) -> str:
        ...
        encoded = jwt.encode(
            headers=headers,
            payload=filter_none(dict(
                iss=self.iss,
                sub=authorization.subject_id,
                aud=self.aud,
                exp=int(authorization.expire_at.timestamp()) if authorization.expire_at else None,
                nbf=int(authorization.not_before.timestamp()) if authorization.not_before else None,
