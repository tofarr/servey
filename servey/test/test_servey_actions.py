...
for action in find_actions():
    for example in (action.action_meta.examples or []):
        if example.include_in_tests:
            test_methods['test_'+action.action_meta.name+'__'+example.name] = _define_action_example_test(action, example)

TestServeyActions = type('TestServeyActions', (TestCase,), test_methods)
