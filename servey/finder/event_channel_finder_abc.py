...
def find_channels() -> Iterator[EventChannelABC]:
    channel_finders = get_impls(EventChannelFinderABC)
    for channel_finder in channel_finders:
        finder = channel_finder()
        yield from finder.find_channels()


def find_channels_by_type(channel_type: Type[T]) -> Iterator[T]:
    for channel in find_channels():
